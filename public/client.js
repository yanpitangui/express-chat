 $(document).ready(function() {
  ...
  socket.on("user", function(data) {
    ...
    var message = `<span style="color:${data.color}" >${data.name}</span>`;
    if (data.connected) {
    ...
    data.listaUsuarios.forEach(element => {
      $("#listaUsuarios").append($("<p>").html(`<b><span style="color:${element.color}" >` + element.name + "</span></b>"));
    });
      ...
      Notification.requestPermission().then(function(permission) {
        if (permission === "granted") {
          var noty = new Notification(data.name, {
            ...
            icon:
              "https://cdn.glitch.com/054ae6c2-367c-4bd7-bc58-3f0aee51ecc4%2Ftoxic-xxl.png?1548695517512" //The URL of an image to be used as an icon
          });
  ...
  function refreshTypingStatus() {
    const ut = usersTyping.map(value => {
      return value.name;
    });
    if (ut.length == 0) {
      typingStatus.html("");
